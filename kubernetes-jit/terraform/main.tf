...
resource "kubiya_knowledge" "k8s_jit_access" {
  name             = "Kubernetes JIT Access Management Guide"
  groups           = var.kubiya_groups_allowed_groups
  description      = "Knowledge base for Kubernetes JIT access management and troubleshooting"
  labels           = ["kubernetes", "jit", "access-management"]
...
resource "kubiya_agent" "jit_guardian" {
  ...
  runner        = var.kubiya_runner
  description   = "AI-powered K8s JIT permissions guardian"
  model         = "azure/gpt-4o"
  instructions  = ""
  sources       = [kubiya_source.enforcer_source.name, kubiya_source.k8s_jit_tools.name]
